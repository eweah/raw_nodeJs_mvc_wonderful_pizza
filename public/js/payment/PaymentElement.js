 ...
 * @requires Templates
 * 
 * @classdesc PaymentElement class for the order page
 * 
 **/

import Templates from './Templates.js';
class PaymentElement extends Templates {
  constructor() {
    super();
    
    // auto bind methods 

    this.autobind(PaymentElement)
  }
...
this.addEventListener('a-guest-has-placed-an-order', event => {
  ...
  const feedback = () => window.location.href = 'https://www.wonderfulpizzas.ericsonweah.com/feedback'
  const cleanup = () => {
...
this.paymentForm().addEventListener('submit', async event => {
  ...
  init(){
    this.onAguestHasPlacedAnOrder()
    this.onAuthHasPlacedAndOrder()
    this.onSelectCard()
